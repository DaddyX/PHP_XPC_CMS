...
// 		var_dump($xml);echo '--';var_dump($url);echo '--';var_dump($timeOut);
		Log::write('xml--'.$xml, 'INFO');
		$response = self::postXmlCurl($xml, $url, false, $timeOut);
		Log::write('response--'.$response, 'INFO');
	...
	{		
		...
		if($data){
			...
			curl_close($ch);
			if ($error == 28){
			    $errorMsg = 'curl出错，请求超时。';
			}
			else {
			    $errorMsg = 'curl出错，错误码:'.$error;
			}
			Log::write('curl出错，错误码:'.$error, 'INFO');
			$returnXml = '<xml>
